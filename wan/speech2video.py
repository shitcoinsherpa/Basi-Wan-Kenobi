...
class WanS2V:
    ...
    def read_last_n_frames(self,
        ...
        """
        from decord import VideoReader  # lazy: only the optional pose path needs it
    ...
    def generate(
        ...
        def _mdump(tag):
            ...
            _f = torch.cuda.mem_get_info()[0] / 1e9
        ...
        def _leakdump(tag):
            ...
            agg = {}
            seen = set()
            for o in _g.get_objects():
                try:
                    if isinstance(o, torch.Tensor) and o.is_cuda:
                        sid = o.storage().data_ptr() if o.storage() is not None else id(o)
                        if sid in seen:
                            continue
                        seen.add(sid)
                        mb = o.element_size() * o.nelement() / 1e6
                        key = (tuple(o.shape), str(o.dtype))
                        v = agg.get(key, [0.0, 0]); v[0] += mb; v[1] += 1; agg[key] = v
                except Exception:
                    continue
            top = sorted(agg.items(), key=lambda kv: -kv[1][0])[:14]
            tot = sum(v[0] for v in agg.values())
            print(f"[s2v-leak] {tag}: {len(agg)} shapes, {tot/1000:.2f}GB live (gc-reachable)",
                  flush=True)
            for (shp, dt), (mb, cnt) in top:
                print(f"[s2v-leak]    {mb/1000:6.2f}GB x{cnt:4d}  {dt:18s} {shp}", flush=True)
        if _memdbg:
            print(f"[s2v-mem] audio_emb shape={tuple(audio_emb.shape)} "
                  f"dtype={audio_emb.dtype} dev={audio_emb.device} "
                  f"GB={audio_emb.element_size()*audio_emb.nelement()/1e9:.3f} "
        ...
        # A FIXED chunk-0 anchor (mean frame) bounds cumulative drift at ANY length
        # without the cost/risk of overlap-window denoise surgery (the measurement did
        # NOT justify that). OFF by default (strength 0 == bit-identical); auto-engaged
        # only at >= color_anchor_min_chunks. mean@0.5 measured strictly better on real
        # output (drift 1.61->1.03, seam 0.59->0.30) and removes 85% of injected drift.
        # (Apache-2.0 change note.)
        _anchor_on = (color_anchor_strength > 0.0
                      and num_repeat >= color_anchor_min_chunks)
        color_anchor = None  # (3,H,W) chunk-0 temporal-mean frame, set on r==0
        ...
        _mdump("after ref_latents encode")
        ...
        with (
            ...
            for r in range(num_repeat):
                _mdump(f"chunk r={r} START")
                _leakdump(f"chunk r={r} START")
                if _snapshot and r == int(_os.environ.get("BASIWAN_S2V_SNAPSHOT_CHUNK", "1")):
                    _sp = _os.environ.get("BASIWAN_S2V_SNAPSHOT_PATH",
                                          "s2v_snapshot.pickle")
                    torch.cuda.memory._dump_snapshot(_sp)
                    torch.cuda.memory._record_memory_history(enabled=None)
                    print(f"[s2v-snap] dumped {_sp} at chunk r={r} START", flush=True)
                ...
                # blocks[:N] resident and streams the rest. (Apache-2.0 change note.)
                if (offload_model or self.init_on_cpu) and not getattr(
                        self.noise_model, "_basiwan_block_swap", False):
                ...
                # multi-chunk run into OOM while a clean single-chunk run fits. The
                # offline sweep only fit because it empty_cache'd before each generate;
                # do the same here so the in-loop path has the same headroom (measured:
                # reserved 12.0 -> ~10.5GB, ~1.5GB recovered per chunk). Negligible cost
                # (one cudaFree of empty blocks); only matters at the high-res edge.
                torch.cuda.empty_cache()

                for i, t in enumerate(tqdm(timesteps)):
                    if _memdbg and i < 3:
                        _mdump(f"chunk r={r} step {i} pre-forward")
                ...
                if offload_model and not getattr(
                        self.noise_model, "_basiwan_block_swap", False):
                    self.noise_model.cpu()
                    torch.cuda.synchronize()
                    torch.cuda.empty_cache()
                _mdump(f"chunk r={r} after step loop")
                ...
                # short clips never reach here (_anchor_on is False) -> bit-identical.
                if _anchor_on:
                    from basi.sliding import match_and_blend_colors
                    if r == 0:
                        color_anchor = image[0].mean(dim=1).detach()  # (3,H,W)
                    elif color_anchor is not None:
                        image = match_and_blend_colors(
                            image[0],
                            color_anchor.to(device=image.device, dtype=image.dtype),
                            color_anchor_strength).unsqueeze(0)
                ...
                # and compound (~+2-3GB/chunk -> OOM by chunk 2-3 at 720p; measured via
                # BASIWAN_S2V_MEMDEBUG). empty_cache alone can't free them (they're
                # referenced). Drop the refs + gc.collect so each chunk starts from the
                # block-swap baseline, not the previous chunk's peak. The loop only did
                # this when offload_model; block-swap needs it too.
                del image, decode_latents, arg_c  # latents/noise freed post-loop
                if guide_scale > 1:
                    del arg_null
                try:
                ...
                # del-ing locals can't reclaim them; they survive the chunk boundary and
                # accumulate (~+2-3GB/chunk -> OOM by chunk 2-3 at 720p; root-caused via a
                # gc live-tensor census). Null them between chunks; the next chunk's
                # forward recomputes them before use, so it's correctness-neutral.
                for _attr in ("pre_compute_freqs", "merged_audio_emb",
                              "audio_emb_global"):
                    if getattr(self.noise_model, _attr, None) is not None:
                        setattr(self.noise_model, _attr, None)
                gc.collect()
                torch.cuda.empty_cache()
                _mdump(f"chunk r={r} END (after motion encode + cleanup)")
