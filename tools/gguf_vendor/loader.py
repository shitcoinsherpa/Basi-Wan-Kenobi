...
# Modified for BASI WAN K3N0B1 from city96/ComfyUI-GGUF (Apache-2.0).
import warnings
import logging
import torch
import gguf
...
def gguf_sd_loader(path, handle_prefix="model.diffusion_model.", is_text_model=False):
    ...
    for sd_key, tensor in tensors:
        ...
        # BF16 is a raw (non-block) dtype the kernel path can't consume, so
        # materialize it to f32 regardless of rank. The original guard only
        # caught 1-D BF16 (norms/scales); the VACE-Fun GGUFs add 2-D BF16
        # tensors (head, text/time embeddings) that fell through this check and
        # crashed bare_gguf. dequantize_tensor reshapes via the stored
        # tensor_shape, so N-D is correct; the big Wan weights are Q4_K/Q6_K
        # (block-quantized) not BF16, so dequanting every BF16 is cheap.
        # [#388 2026-06-11]
