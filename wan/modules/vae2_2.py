...
class RMS_norm(nn.Module):
    ...
    def forward(self, x):
        # [Faster-Wan2.2 P6.F3] Cast gamma/bias to x.dtype to avoid FP32 promotion
        # leak. Stock multiplied F.normalize(x) (bf16) * scale (Python float) * gamma
        # (FP32 param) → result becomes FP32 → next op gets FP32 → silent dtype
        # promotion through ~50 RMS_norms in the decoder. Cast once per call (cheap)
        # to preserve x.dtype. Eliminates ~470 MB redundant FP32 alloc per VAE pass.
...
class Upsample(nn.Upsample):
    ...
    def forward(self, x):
        """
        [Faster-Wan2.2 P6.F2] Drop FP32 upcast — PyTorch >=2.1 supports BF16
        for nearest/nearest-exact F.interpolate. The original cast was a defensive
        upgrade from older PyTorch; on 2.4+ it's pure overhead. Allocating a
        FP32 copy of [bt, 160, 720, 1280] = ~470 MB per call × 3 ups × 21 frames
        per 720p decode = ~30 GB of useless HBM traffic per video.
        """
        # Try native BF16; fallback only if a future PyTorch breaks it.
        try:
            return super().forward(x)
        except RuntimeError:
...
class WanVAE_(nn.Module):
    ...
    def __init__(
        ...
        # Decoder upsamples spatially at every block except the last. Tile
        # parameters below are expressed in decoder-output pixels before the
        # final unpatchify(patch_size=2), so convert them back to latent units.
        self._decode_patch_size = 2
        self._decode_out_channels = 3 * self._decode_patch_size**2
        self._decoder_spatial_scale = 2**max(0, len(dim_mult) - 1)
        # Latent-area split chosen so p480_17f (30x52=1560) stays on the fast
        # untiled path while p720_17f (45x80=3600) auto-tiles.
    ...
    def _decode_full(self, z):
        iter_ = z.shape[2]
        x = self.conv2(z)
        # [Faster-Wan2.2 P6.F5] Replace O(N²) growing torch.cat with list+single-cat.
        # Stock: iter=21 → 20 cats with LHS growing from 1 to 20 frames → ~10 GB total
        # copy traffic for 720p output (mostly wasted HBM). New: collect frames in list,
        # single torch.cat at end → O(N) traffic.
        outs = []
        for i in range(iter_):
            self._conv_idx = [0]
        ...
        out = torch.cat(outs, dim=2) if iter_ > 1 else outs[0]
        return unpatchify(out, patch_size=self._decode_patch_size)
    ...
    def _decode_tiled(self, z, tile_h=256, tile_w=256, overlap=32):
        """Spatial tiled decode with 2D Hann blending.

        tile_h/tile_w/overlap follow the design memo and are specified in the
        decoder's output-pixel space before the final unpatchify(patch_size=2).
        """
        x = self.conv2(z)
        ...
        tile_coords = [(h0, w0) for h0 in h_starts for w0 in w_starts]
        tile_feat_maps = [[None] * self._conv_num for _ in tile_coords]
        window_cache = {}
        outs = []

        for i in range(iter_):
            frame_accum = torch.zeros(
                (b, self._decode_out_channels, 1, out_h, out_w),
                device=x.device,
                dtype=acc_dtype,
            )
            frame_weight = torch.zeros(
                (1, 1, 1, out_h, out_w),
                device=x.device,
                dtype=acc_dtype,
            )

            for tile_idx, (h0, w0) in enumerate(tile_coords):
                h1 = min(h0 + tile_h_latent, latent_h)
                w1 = min(w0 + tile_w_latent, latent_w)
                conv_idx = [0]
                decoded = self.decoder(
                    x[:, :, i:i + 1, h0:h1, w0:w1],
                    feat_cache=tile_feat_maps[tile_idx],
                    feat_idx=conv_idx,
                    first_chunk=(i == 0),
                )
                ...
                frame_accum[:, :, :, out_h0:out_h1, out_w0:out_w1] += decoded_acc
                frame_weight[:, :, :, out_h0:out_h1, out_w0:out_w1] += window

            eps = torch.finfo(frame_weight.dtype).eps
            frame = (frame_accum / frame_weight.clamp_min(eps)).to(x.dtype)
            outs.append(frame)
