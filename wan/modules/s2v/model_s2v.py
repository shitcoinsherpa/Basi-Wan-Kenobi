...
class WanS2VAttentionBlock(WanAttentionBlock):
    ...
    def forward(self, x, e, seq_lens, grid_sizes, freqs, context, context_lens):
        ...
        # at seq=24640 via the memory snapshot). This mirrors model.py WanAttentionBlock's
        # PROVEN default P31/P34 bf16-mod path (BASIWAN_FORCE_BF16_MOD=1, the canonical
        # ship recipe), adapted to the S2V block's 2-segment (seg_idx) structure: do the
        # tiny (modulation+e0) add + the norm*scale+shift + gate-multiply in bf16, no
        # .float()/autocast. The e0/modulation tensors are tiny ([1,6,2,dim]) so the add
        # is cheap; the win is dropping the seq-sized fp32 upcasts. Gated by the SAME env
        # as model.py for consistency; default ON for bf16 input. (Apache-2.0 change.)
        import os as _os
        _bf16_mod = (_os.environ.get("BASIWAN_NO_BF16_MOD") != "1") and (
            _os.environ.get("BASIWAN_FORCE_BF16_MOD", "1") == "1"
            or x.dtype == torch.bfloat16)
        if _bf16_mod:
            _e = e[0].to(x.dtype)                          # (1,6,2,dim) bf16
            _mod = self.modulation.unsqueeze(2).to(x.dtype)  # (1,6,1,dim) bf16
            eb = [el.squeeze(1) for el in (_mod + _e).chunk(6, dim=1)]  # 6 x (1,2,dim)
            n1 = self.norm1(x)                             # bf16, NO .float()
            nx = torch.cat([n1[:, seg_idx[i]:seg_idx[i + 1]] * (1 + eb[1][:, i:i + 1])
                            + eb[0][:, i:i + 1] for i in range(2)], dim=1)
            y = self.self_attn(nx, seq_lens, grid_sizes, freqs)
            x = x + torch.cat([y[:, seg_idx[i]:seg_idx[i + 1]] * eb[2][:, i:i + 1]
                               for i in range(2)], dim=1)

            def cross_attn_ffn_bf16(x):
                x = x + self.cross_attn(self.norm3(x), context, context_lens)
                n2 = self.norm2(x)                         # bf16, NO .float()
                y2 = self.ffn(torch.cat(
                    [n2[:, seg_idx[i]:seg_idx[i + 1]] * (1 + eb[4][:, i:i + 1])
                     + eb[3][:, i:i + 1] for i in range(2)], dim=1))
                x = x + torch.cat([y2[:, seg_idx[i]:seg_idx[i + 1]] * eb[5][:, i:i + 1]
                                   for i in range(2)], dim=1)
                return x
            return cross_attn_ffn_bf16(x)

        # ---- legacy fp32 modulation path (BASIWAN_NO_BF16_MOD=1) ----
        e = e[0]
